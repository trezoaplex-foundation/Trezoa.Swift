...
extension Action {
    ...
    fileprivate func signAndSend(mintAddress: String,
        ...
        guard let mintAddress = PublicKey(string: mintAddress) else {
            onComplete(.failure(TrezoaError.unauthorized))
            return
        }
...
@available(macOS 10.15, *)
public extension Action {
    func findTPLTokenDestinationAddress(
        mintAddress: String,
        destinationAddress: String
    ) async throws -> TPLTokenDestinationAddress {
        try await withCheckedThrowingContinuation { c in
            self.findTPLTokenDestinationAddress(mintAddress: mintAddress, destinationAddress: destinationAddress, onComplete: c.resume(with:))
        }
    }
}
