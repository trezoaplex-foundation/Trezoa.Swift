...
extension Action {
    ...
    public func swap(
            ...
            getPoolRequest = ContResult<Pool, Error>.init { cb in
                self.getSwapPools { poolsResult in
                    ...
                    case .success(let pools):
                        ...
                        cb(.failure(TrezoaError.other("Unsupported swapping tokens")))
                        return
        ...
        getPoolRequest.flatMap { pool in
            ...
            guard let pool = pool,
                ...
                return .failure(TrezoaError.other("Swap pool is not valid"))
            }
            // get variables
            ...
            guard let userTransferAuthority = HotAccount() else {
                return .failure(TrezoaError.other("Unsupported swapping tokens"))
            }
            ...
            if tokenAInfo.isNative {
                guard let newAccount = try? self.createWrappedSolAccount(
                    ...
                    return .failure(TrezoaError.other("Could not create Wrapped SolAccount"))
                }
            ...
            let isMintBWTRZ = destinationMint == .wrappedSOLMint
            if destination == nil || isMintBWTRZ {
        ...
        self.api.getAccountInfo(account: account, decodedTo: AccountInfo.self) { accountInfoResult in
            ...
            case .success(let account):
                ...
                if account.owner != tokenProgramId.base58EncodedString {
                    onComplete(.failure(TrezoaError.other("Invalid account owner")))
                    return
                }
                ...
                onComplete(.failure(TrezoaError.other("Invalid data")))
                return
            case .failure(let error):
    ...
    private func createWrappedSolAccount(
        ...
        guard let newAccount = HotAccount() else {
            return .failure(TrezoaError.invalidRequest(reason: "Could not create new Account"))
        }
