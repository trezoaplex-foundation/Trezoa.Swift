...
public extension Api {
        ...
        router.request(parameters: [serializedTransaction, configs]) { (result: Result<TransactionID, Error>) in
            ...
            case .failure(let error):
                if let trezoaError = error as? TrezoaError {
                    onComplete(.failure(self.handleError(error: trezoaError)))
                    return
                } else {
    ...
    fileprivate func handleError(error: TrezoaError) -> Error {
        if case .invalidResponse(let response) = error,
           response.message != nil {
            ...
            return TrezoaError.invalidResponse(ResponseError(code: response.code, message: message, data: response.data))
        }
        return error
    }
}
