...
extension PublicKey {
    ...
    public static func findProgramAddress(
        ...
        return .failure(TrezoaError.notFoundProgramAddress)
    }
    ...
    private static func createProgramAddress(
        ...
        for seed in seeds {
            if seed.bytes.count > maxSeedLength {
                return .failure(TrezoaError.other("Max seed length exceeded"))
            }
            data.append(seed)
        }
        ...
        guard let publicKeyBytes = Bignum(number: hash.hexString, withBase: 16)?.data else {
            return .failure(TrezoaError.other("Can not get Bytes"))
        }

        // check it
        if isOnCurve(publicKeyBytes: publicKeyBytes).toBool() {
            return .failure(TrezoaError.other("Invalid seeds, address must fall off the curve"))
        }
