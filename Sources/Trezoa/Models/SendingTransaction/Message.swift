...
extension Transaction.Message {
    ...
    static func from(buffer: Data) throws -> Transaction.Message {
        ...
        guard let numRequiredSignatures = byteArray.first else { throw TrezoaError.invalidRequest(reason: "Could not parse number of required signatures") }
        byteArray = Data(byteArray.dropFirst())
        
        guard let numReadonlySignedAccounts = byteArray.first else { throw TrezoaError.invalidRequest(reason: "Could not parse number of signed accounts") }
        byteArray = Data(byteArray.dropFirst())
        
        guard let numReadonlyUnsignedAccounts = byteArray.first else { throw TrezoaError.invalidRequest(reason: "Could not parse number of unsigned accounts") }
        byteArray = Data(byteArray.dropFirst())
