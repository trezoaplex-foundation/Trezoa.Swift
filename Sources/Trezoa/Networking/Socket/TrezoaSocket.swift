...
enum TrezoaSocketError: Error {
    case disconnected
    case couldNotSerialize
}
public protocol TrezoaSocketEventsDelegate: AnyObject {
    func connected()
...
public protocol TrezoaWebSocketEvents: AnyObject {
...
public class TrezoaSocket {
    private var socket: WebSocket?
    private var enableDebugLogs: Bool
    private var request: URLRequest
    private weak var delegate: TrezoaSocketEventsDelegate?
    ...
    public func start(delegate: TrezoaSocketEventsDelegate) {
    ...
    public func accountSubscribe(publickey: String) -> Result<String, Error> {
        ...
        let request = TrezoaRequest(method: method.rawValue, params: params)
    ...
    private func writeToSocket(request: TrezoaRequest) -> Result<String, Error> {
        guard let jsonData = try? JSONEncoder().encode(request) else { return Result.failure(TrezoaSocketError.couldNotSerialize) }
        guard let socket = socket else { return Result.failure(TrezoaSocketError.disconnected) }
        socket.write(data: jsonData)
        return Result.success(request.id)
    }
}

extension TrezoaSocket: WebSocketDelegate {
