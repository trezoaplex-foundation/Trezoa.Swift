...
class createAssociatedTokenAccount: XCTestCase {
    ...
    func testGetOrCreateAssociatedTokenAccount() {
        ...
        // act
        let account: (transactionId: TransactionID?, associatedTokenAddress: PublicKey)? = try! trezoa.action.getOrCreateAssociatedTokenAccount(for: signer.publicKey, tokenMint: tokenMint, payer: signer)?.get()
    ...
    func testFailCreateAssociatedTokenAccountItExisted() {
        let tokenMint = PublicKey(string: "2tWC4JAdL4AxEFJySziYJfsAnW2MHKRo98vbAPiRDSk8")!
        XCTAssertThrowsError(try (trezoa.action.createAssociatedTokenAccount(for: signer.publicKey, tokenMint: tokenMint, payer: signer)?.get() as TransactionID?))
    }
