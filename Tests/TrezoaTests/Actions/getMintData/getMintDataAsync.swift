...
class getMintDataAsync: XCTestCase {
    ...
    func testGetMintData() async throws {
        let data: Mint? = try await trezoa.action.getMintData(mintAddress: PublicKey(string: "8wzZaGf89zqx7PRBoxk9T6QyWWQbhwhdU555ZxRnceG3")!)
        XCTAssertNotNil(data)
    }
    
    func testGetMultipleMintDatas() async throws {
        let datas: [PublicKey: Mint]? = try await trezoa.action.getMultipleMintDatas(mintAddresses: [PublicKey(string: "8wzZaGf89zqx7PRBoxk9T6QyWWQbhwhdU555ZxRnceG3")!])
        XCTAssertNotNil(datas)
    }
    
    func testGetPools() async throws {
        let pools: [Pool]? = try await trezoa.action.getSwapPools()
