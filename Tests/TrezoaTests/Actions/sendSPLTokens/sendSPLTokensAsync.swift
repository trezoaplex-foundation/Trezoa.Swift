...
class sendTPLTokensAsync: XCTestCase {
    ...
    func testSendTPLTokenWithFee() async throws {
        ...
        let transactionId = try await trezoa.action.sendTPLTokens(
        ...
        let transactionIdB = try await trezoa.action.sendTPLTokens(
            mintAddress: mintAddress,
            decimals: 5,
            from: destination,
            to: source,
