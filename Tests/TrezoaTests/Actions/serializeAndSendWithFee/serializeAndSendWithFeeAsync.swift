...
class serializeAndSendWithFeeAsync: XCTestCase {
    ...
    func testSimulationSerializeAndSend() async throws {
        ...
        let balance = try await trezoa.api.getBalance(account: signer.publicKey.base58EncodedString)
        ...
        let transactionId = try await trezoa.action.serializeAndSendWithFeeSimulation(instructions: [instruction], signers: [signer])
    ...
    func testSerializeAndSend() async throws {
        ...
        let transactionId = try await trezoa.action.serializeAndSendWithFee( instructions: [instruction], signers: [signer])
        XCTAssertNotNil(transactionId)
    }
