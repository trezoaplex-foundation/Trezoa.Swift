...
class swap: XCTestCase {
    ...
    /*func testSwapToken() {
        ...
        let source = try Trezoa.PublicKey(string: USDCWallet)
        let sourceMint = try Trezoa.PublicKey(string: USDCMintAddress)
        let destination = try Trezoa.PublicKey(string: USDTWallet)
        let destinationMint = try Trezoa.PublicKey(string: USDTMintAddress)

        _ = try trezoaSDK.swap(
            source: source,
